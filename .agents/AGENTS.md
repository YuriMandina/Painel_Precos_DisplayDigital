...
1. **Brainstorm Obrigatório (via Skills):** Antes de propor qualquer alteração arquitetural, mudança em banco de dados ou reestruturação de views complexas, você DEVE invocar as skills globais (como `multi-agent-brainstorming` ou `design-orchestration`) para executar um processo de brainstorm profundo e documentado, apresentando opções prós e contras ao usuário antes de codar.
